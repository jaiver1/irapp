                ...
                <div class="card-body d-sm-flex justify-content-between">
                    ...
                    data-toggle="tooltip" data-placement="bottom" title="Lista de ventas">
                      <i class="fas fa-2x fa-tags"></i>
                             ...
                             <a href="{{ route('ventas.edit', $venta->id) }}" class="btn btn-outline-warning btn-circle waves-effect hoverable" 
                    data-toggle="tooltip" data-placement="bottom" title='Editar la venta #{{ $venta->id }}'>
                      <i class="fas fa-2x fa-pencil-alt"></i>
                            </a>

                                    <a onclick="eliminar_venta({{ $venta->id }})"  class="btn btn-outline-danger btn-circle waves-effect hoverable" 
                    data-toggle="tooltip" data-placement="bottom" title='Eliminar la venta #{{ $venta->id }}'>
                      <i class="fas fa-2x fa-trash-alt"></i>
                            </a>
                            <form id="eliminar{{ $venta->id }}" method="POST" action="{{ route('ventas.destroy', $venta->id) }}" accept-charset="UTF-8">
...
<div class="list-group hoverable">
  <a class="list-group-item active z-depth-2 white-text waves-light hoverable">
      <i class="fas fa-tags  mr-2"></i><strong>Orden #{{ $venta->id }}</strong>
    </a>
...
function reload_datatable(){
    var url_send = "{{ route('ventas.getDetalles',array($venta->id)) }}";
...
$(document).ready(function() {
    reload_datatable();
    var url_send = "{{ route('ventas.formDetalles',array($venta->id,0)) }}";
