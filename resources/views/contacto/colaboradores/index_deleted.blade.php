            ...
            <div class="card mb-4 wow fadeIn hoverable">
                ...
                <div class="card-body d-sm-flex justify-content-between">
                    ...
                    <a href="{{ route('colaboradores.index') }}">Lista de colaboradores</a>
                        <span>/</span>
                        <span> @if ($colaboradores->count() === 1)
                Un colaborador eliminado
            @elseif ($colaboradores->count() > 1)
                {{ $colaboradores->count() }} colaboradores eliminados
            @else
               No hay colaboradores eliminados
            @endif
            </span>
                    </h4>
      ...
      <a onclick="restaurar_colaborador({{ $colaborador->id }},'{{$colaborador->persona->primer_nombre}} {{$colaborador->persona->primer_apellido}}')" class="text-success m-1" 
                    ...
                    data-toggle="tooltip" data-placement="bottom" title='Eliminar definitivamente el colaborador "{{$colaborador->persona->primer_nombre}} {{$colaborador->persona->primer_apellido}}"'>
                      <i class="fas fa-2x fa-trash"></i>
                            ...
                            <form id="restaurar{{ $colaborador->id }}" method="POST" action="{{ route('colaboradores.deleted.update', $colaborador->id) }}" accept-charset="UTF-8">
    <input name="_method" type="hidden" value="PUT">
    {{ csrf_field() }}
</form>
                            <form id="eliminar{{ $colaborador->id }}" method="POST" action="{{ route('colaboradores.deleted.destroy', $colaborador->id) }}" accept-charset="UTF-8">
