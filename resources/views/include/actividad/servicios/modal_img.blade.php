    ...
    <div class="modal-dialog modal-lg modal-notify modal-secondary" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h3 class="heading lead"><i class="white-text fas fa-images mr-1"></i>Imagenes de "{{ $servicio->nombre }}"</h3>
            ...
            <div class="modal-body">
                <div class="text-center">
                    ...
                    <h3> Imagenes de "{{ $servicio->nombre }}"</h3>
                </div>
                <hr/>
            <!--Carousel Wrapper-->
<div id="carousel_imagenes" class="carousel slide carousel-fade hoverable div-border" data-ride="carousel">
    ...
    <ol class="carousel-indicators">
      ...
      @for ($i = 1; $i < $servicio->imagenes->count(); $i++)
    ...
    <div class="carousel-item imagen-carousel-item active">
          ...
          <div class="view imagen-carousel-img">
              ...
              <img src="{{ asset($servicio->imagenes->first()->ruta) }}" class="img-fluid rounded img-thumbnail"  alt="{{ $servicio->imagenes->first()->nombre }}" onerror=this.src="{{ asset('img/dashboard/servicios/404.png')  }}">
              @else
              <img src="{{ asset('img/dashboard/servicios/404.png')  }}" class="img-fluid rounded img-thumbnail" alt="404">
              @endif
                <div class="mask rgba-white-slight"></div>
          </div>
          <div class="carousel-caption imagen-carousel-caption">
              @if($servicio->imagenes->count())
            <h3 class="h3-responsive">{{ $servicio->imagenes->first()->nombre }}</h3>
            <p>{{ $servicio->imagenes->first()->nombre }}</p>
            @else
            <h3 class="h3-responsive">No hay imagenes</h3>
            <p>El servicio "{{ $servicio->nombre }}" no tiene imagenes.</p>
            @endif
          </div>
        </div>
    @for ($i = 1; $i < $servicio->imagenes->count(); $i++)
    ...
    <div class="carousel-item imagen-carousel-item">
      <!--Mask color-->
      <div class="view imagen-carousel-img">
              <img src="{{ asset($servicio->imagenes[$i]->ruta) }}" class="img-fluid rounded img-thumbnail"  alt="{{ $servicio->imagenes[$i]->nombre }}" onerror=this.src="{{ asset('img/dashboard/servicios/404.png')  }}">
            <div class="mask rgba-white-slight"></div>
      </div>
      <div class="carousel-caption imagen-carousel-caption">
        <h3 class="h3-responsive">{{ $servicio->imagenes[$i]->nombre }}</h3>
        <p>{{ $servicio->imagenes[$i]->nombre }}</p>
      </div>
    </div>
